...
export default function TabResults({ results }: TabResultsProps) {
  ...
  return (
    <div className="rounded-xl bg-cyberteal-surface shadow-cyber">
          ...
          >
            <span
              className={`mr-2 transition-all duration-200 ${
                activeTab === tab.id ? "text-cyberteal-primary" : "text-cyberteal-muted"
              }`}
            >
              {tab.icon}
            </span>
      ...
      {/* Tab Content */}
      <div className="p-6">
        {activeTab === "use-cases" && <UseCaseTab useCases={results.useCases} />}
        {activeTab === "tokenomics" && <TokenomicsTab tokenomics={results.tokenomics} />}
        {activeTab === "roadmap" && <RoadmapTab roadmap={results.roadmap} />}
        {activeTab === "tools" && <ToolsTab toolCategories={results.tools} />}
        {activeTab === "launchpad" && <LaunchpadTab guide={results.launchpadGuide} />}
      </div>
    </div>
  )
}
